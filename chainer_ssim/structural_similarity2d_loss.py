...
def create_window(window_size, channel, xp):
    ...
    x_window = weight.reshape(1, 1, 1, -1) # (out_ch, in_ch, y, x)
    x_window = xp.repeat(x_window, channel, axis=0)

    y_window = weight.reshape(1, 1, -1, 1) # (out_ch, in_ch, y, x)
...
def gaussian_filter(img, window, pad, channel):
    ...
    h = F.convolution_2d(img, x_window, pad=(0, pad), groups=channel)
    return F.convolution_2d(h, y_window, pad=(pad, 0), groups=channel)
...
def structural_similarity2d_loss(img1, img2, window_size = 11, data_range=1, size_average = True):
